...
public class TextReportModel13FMI extends FTPRealseModel implements Text_Report {
	@Override
	public void Write_text(String CustomerCode, String Device, String Lot, String CP, File DataSorce, String FileName)
			throws IOException {
		// TODO Auto-generated method stub
		TextReportModel13FMI model13=new TextReportModel13FMI();
			...
			for (int i = 0; i < Row_R; i++) {
				for (int j = 0; j < Col_R; j++) {
					if (MapCell_R[i][j]==null) {
						if (judgeCoordinate(i, j, MapCell_R)) {
							out.print("X");	
						}else {
							out.print(".");	
						}
	...
	private boolean judgeCoordinate(Integer i,Integer j,String[][] MapCell_R)
	{
		try {
			if (MapCell_R[i+2][j]!=null&&MapCell_R[i][j+2]!=null&&MapCell_R[i-2][j]!=null&&MapCell_R[i][j-2]!=null) {
				return true;
			}
		} catch (Exception e) {
			// TODO: handle exception
