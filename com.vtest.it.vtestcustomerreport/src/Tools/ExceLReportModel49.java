...
public class ExceLReportModel49 extends Report_Model {

	private static final File Model=new File("/Config/HS9686C1B1.CP_Format.xlsx");
	private final Integer[] Bin_Array={1,21,2,3,4};
	public ExceLReportModel49() throws IOException {
		super(Model);
		...
		try {
			...
			for (int i =0; i< Prober_Mappings.length; i++) {
				try {
					...
					Rows.getCell(8).setCellFormula("SUM(E"+(RightID_R+7)+":H"+(RightID_R+7)+")");
