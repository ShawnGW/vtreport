...
public class TextReportModel4s extends FTPRealseModel implements Text_Report {

	@Override
	public void Write_text(String CustomerCode, String Device, String Lot, String CP, File DataSorce,String FileName)
			throws IOException {
		// TODO Auto-generated method stub
		TextReportModel4 model1=new TextReportModel4();
		...
		for (int k = 0; k < Filelist.length; k++) {			
			...
			try {
				String pre=FinalName.split("_")[0];
				...
				for (File file : files) {
					if (file.getName().endsWith(suffix)&&file.getName().contains(pre)) {
						file.delete();
					}
				}
			} catch (Exception e) {
			...
			for (int i =0 ; i <Row_R ; i++) {
				int rowNumber=i+1;
				if (rowNumber<10) {
					out.print("00"+rowNumber);
				}else if (rowNumber>9&&rowNumber<100) {
					out.print("0"+rowNumber);
				}else {
					out.print(rowNumber);
				}
				out.print("   ");
