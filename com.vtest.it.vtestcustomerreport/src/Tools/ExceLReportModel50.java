...
public class ExceLReportModel50 extends Report_Model {

	private static final File Model=new File("/Config/FSC_S31_Format.xlsx");
	private final Integer[] Bin_Array={1,2,3,6,4,5,7,8,9,10,11,12,13,14,15,99
};
	public ExceLReportModel50() throws IOException {
		super(Model);
	...
	public void Write_Excel(String CustomerCode, String Device, String Lot, String CP, File file, File Result_Excel) {
		...
		try {
			...
			Row_Summary.getCell(1).setCellValue(Local_lot);
			Row_Summary.getCell(0).setCellValue(Device);
			Row_Summary.getCell(3).setCellValue(MES_GrossDie);
			Row_Summary.getCell(2).setCellValue(file.listFiles().length);
			Row_Summary.getCell(16).setCellValue(program);
			...
			for (int i =0; i< Prober_Mappings.length; i++) {
				try {
					...
					Rows.getCell(19).setCellFormula("SUM(E"+(RightID_R+7)+":S"+(RightID_R+7)+")");
					XSSFSheet ID_Sheet=workbook.getSheet("Map");
					ID_Sheet.setDefaultColumnWidth(1);	
					ID_Sheet.setZoom(1, 5);
					int Max_Row=0;
