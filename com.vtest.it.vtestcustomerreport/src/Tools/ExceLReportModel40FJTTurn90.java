...
public class ExceLReportModel40FJTTurn90 extends Report_Model {

	private static final File Model=new File("/Config/GXLX_Format_Uodate.xlsx");
	private final Integer[] Bin_Array={1,999,21,22,23,24,25,26,27,31,32,33,34,35,36,37,38,39,41,42,43,44,45,71,72,73,74,75,76,77,78,81,82,83,84,85,86,87,88,89,91,92,93,94,95,96,97,601,602,603,604,605,606,607,608,609,610,611,612,613,614,615,616,617,618,619,620,621,622,623,624
};
	public ExceLReportModel40FJTTurn90() throws IOException {
		super(Model);
	...
	public void Write_Excel(String CustomerCode, String Device, String Lot, String CP, File file, File Result_Excel) {
		...
		try {			
			...
			Row_Summary.getCell(9).setCellValue(Local_lot);
			Row_Summary.getCell(0).setCellValue(Device);
			Row_Summary.getCell(24).setCellValue(MES_GrossDie);
			Row_Summary.getCell(19).setCellValue(file.listFiles().length);
			Row_Summary.getCell(62).setCellValue(propertiesFirst.get("Tester Program"));
			...
			for (int i =0; i< Prober_Mappings.length; i++) {
				try {
					...
					String MapCols_R=properties.get("TestDieCol");
					String MapRows_R=properties.get("TestDieRow");
					Integer mapRow=Integer.valueOf(MapRows_R);
					Integer mapCol=Integer.valueOf(MapCols_R);
					...
					Map_R=TurnNighteenDegree.Turn(Map_R, mapRow, mapCol);
					
					String temp=MapRows_R;
					MapRows_R=MapCols_R;
					MapCols_R=temp;
