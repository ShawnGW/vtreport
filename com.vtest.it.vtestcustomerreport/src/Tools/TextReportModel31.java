...
public class TextReportModel31 extends FTPRealseModel implements Text_Report{
	...
	public void Write_text(String CustomerCode, String Device, String Lot, String CP, File DataSorce,String FileName)
		...
		File directory=new File(reportBath+CustomerCode+"/"+Device+"/"+Lot+"/"+CP+"/"+Lot+"-EQC");
		if (!directory.exists()) {
			directory.mkdirs();
		}
		File  MergeSummary=new File(reportBath+CustomerCode+"/"+Device+"/"+Lot+"/"+CP+"/"+Lot+"-EQC/"+Lot+"_EQC_SUMMARY.txt");
...
//		ArrayList<String> ResultSummary=new ArrayList<>();
		...
		for (int k = 0; k < Filelist.length; k++) {	
			File CP1RawData=new File(Filelist[k].getPath().replace("CP2", "CP1"));
...
//			if (!Result_Text.getParentFile().exists()) {
//				Result_Text.getParentFile().mkdirs();
//			}
					...
					try {
						if (MapCell_R[i][j]==null) {
							...
							if (Failkeyset.contains((i+up)+":"+(j+left))) {
								...
								{
									FailPointsArray.add((i+up)+","+(j+left)+"  "+FailDieMap.get((i+up)+":"+(j+left))+"/1");
									out.print("X");
									mergefailsum++;
								}
								else
