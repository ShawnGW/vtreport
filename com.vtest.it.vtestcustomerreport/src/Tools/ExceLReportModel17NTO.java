...
public class ExceLReportModel17NTO extends Report_Model {

	private static final File Model=new File("/Config/NTO_QC_New.xlsx");
	public ExceLReportModel17NTO() throws IOException {
		super(Model);
		...
		try {			
			...
			for (int j = 0; j < 66; j++) {
				if(j<23)
				{
					...
					Total_Row.getCell(3+j).setCellFormula("SUM(B"+(char)(j+16)+"7:B"+(char)(j+16)+"31)");
			}
			...
			for (int i =0; i< Prober_Mappings.length; i++) {
				try {
					...
					for (int j = 1; j < 64; j++) {
						XSSFCell Bin_Cell=rows.getCell(4+j);
					...
					rows.getCell(69).setCellValue(Time);
					rows.getCell(70).setCellValue(OPerater_R);
					rows.getCell(71).setCellValue(Tester_R);
					rows.getCell(72).setCellValue(Prober_R);
					rows.getCell(73).setCellValue(ProberCard_R);
					if (Notch_R.equals("0-Degree")) {
						rows.getCell(75).setCellValue("上");
					}else if (Notch_R.equals("90-Degree")) {
						rows.getCell(75).setCellValue("右");//下左
					}else if (Notch_R.equals("180-Degree")) {
						rows.getCell(75).setCellValue("下");//下左
					}else if (Notch_R.equals("270-Degree")) {
						rows.getCell(75).setCellValue("左");//下
					}
					
					rows.getCell(76).setCellValue("V019");
				} catch (Exception e) {
